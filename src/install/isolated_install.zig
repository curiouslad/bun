    ...
    const store: Store = store: {
        ...
        defer pkg_has_transitive_peers.deinit(lockfile.allocator);
        {
            ...
            if (reverse_deps_edges.items.len > 0) {
                ...
                // Assert that edge counts fit in u32 to prevent truncation in release builds
                bun.debugAssert(reverse_deps_edges.items.len <= std.math.maxInt(u32));
        ...
        // Temporary buffer for peer deps to queue after peer dedup check.
        // Peers are buffered here instead of being queued directly to prevent
        // duplicate children when matched_node dedup is triggered.
        var deferred_peers: std.ArrayListUnmanaged(QueuedNode) = .empty;
        defer deferred_peers.deinit(lockfile.allocator);
        ...
        next_node: while (node_queue.readItem()) |entry| {
            // Clear deferred buffers at the start of each iteration to ensure no stale
            // items leak into the peer-dedup check, regardless of early exits.
            deferred_deps.clearRetainingCapacity();
            deferred_peers.clearRetainingCapacity();
            ...
            // Pre-compute whether this package has transitive peers to avoid duplicate lookups
            const has_transitive_peers = pkg_has_transitive_peers.isSet(entry.pkg_id);

            if (entry.dep_id != invalid_dependency_id) {
                const entry_dep = dependencies[entry.dep_id];
                if (pkg_deps.len == 0 or entry_dep.version.tag == .workspace or
                    !has_transitive_peers)
                dont_dedupe: {
            ...
            for (peer_dep_ids.items) |peer_dep_id| {
                ...
                if (visited_parent_node_ids.items.len != 0) {
                    ...
                    const peer_item = QueuedNode{
                        .parent_id = node_id,
                        .dep_id = peer_dep_id,
                        .pkg_id = resolved_pkg_id,
                    };
                    // Buffer peer deps to prevent duplicate children when matched_node
                    // dedup is triggered. They'll be flushed after the dedup check.
                    try deferred_peers.append(lockfile.allocator, peer_item);
                }
            }
            ...
            if (deferred_deps.items.len > 0 or deferred_peers.items.len > 0) {
                ...
                if (matched_node) |existing_id| {
                    ...
                    for (deferred_peers.items) |queued| {
                        try node_queue.writeItem(queued);
                    }
                }
            }
